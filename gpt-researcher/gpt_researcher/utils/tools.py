    ...
    try:
        ...
        if hasattr(response, 'tool_calls') and response.tool_calls:
            ...
            for tool_call in response.tool_calls:
                ...
                available_tool_names = {getattr(tool, "name", None) for tool in tools if getattr(tool, "name", None)}
                matched_tool = next((tool for tool in tools if getattr(tool, "name", None) == tool_name), None)
                if matched_tool is None:
                    logger.warning(
                        "Tool %s not found in bound tools. Available tool names: %s",
                        tool_name,
                        sorted(available_tool_names),
                    )
                    tool_result = (
                        f"Tool '{tool_name}' was not available in the current tool set. "
                    ...
                    try:
                        if hasattr(matched_tool, 'ainvoke'):
                            ...
                            tool_result = await matched_tool(**tool_args) if asyncio.iscoroutinefunction(matched_tool) else matched_tool(**tool_args)
