...
class ResearchConductor:
    ...
    async def conduct_research(self):
        ...
        if self.researcher.mcp_configs and not any(
            "mcp" in name.lower() or "mcpretriever" in name.lower()
            for name in retriever_names
        ):
            self.logger.warning(
                "MCP configs are present but no MCP retriever is active; forcing MCP retriever to preserve MCP config."
            )
            if self.researcher.cfg.retrievers:
                current_retrievers = (
                    list(self.researcher.cfg.retrievers)
                    if isinstance(self.researcher.cfg.retrievers, list)
                    else [r.strip() for r in str(self.researcher.cfg.retrievers).split(",") if r.strip()]
                )
            else:
                current_retrievers = []
            if "mcp" not in [r.lower() for r in current_retrievers]:
                current_retrievers.append("mcp")
            self.researcher.cfg.retrievers = current_retrievers
            self.researcher.retrievers = get_retrievers(self.researcher.headers, self.researcher.cfg)
            retriever_names = [r.__name__ for r in self.researcher.retrievers]
            self.logger.info(f"Forced active retrievers: {retriever_names}")
