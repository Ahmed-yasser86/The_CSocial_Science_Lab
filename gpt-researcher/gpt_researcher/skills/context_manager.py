...
class ContextManager:
    ...
    async def get_similar_content_by_query(self, query: str, pages: list) -> str:
        ...
        # Safety: ensure token rate limiting is applied here as a guard in case
        # callers skipped the ResearchConductor-level limiter. This uses the
        # ResearchConductor's _maybe_rate_limit helper when available so we
        # centralize limiter state and avoid duplicating logic.
        try:
            if hasattr(self.researcher, "research_conductor") and hasattr(self.researcher.research_conductor, "_maybe_rate_limit"):
                await self.researcher.research_conductor._maybe_rate_limit(pages)
        except Exception:
            # Non-fatal: if rate-limiter check fails, proceed — the caller may
            # already have protected the request or another pipeline will apply throttling.
            pass
