...
async def handle_json_error(response: str | None):
    ...
    try:
        ...
        # json_repair may return non-dict structures (e.g., a list). Normalize
        # to find a usable dict containing the expected keys.
        if isinstance(agent_dict, dict):
            candidate = agent_dict
        elif isinstance(agent_dict, list):
            # Prefer the first dict-like item that contains the required keys
            candidate = None
            for item in agent_dict:
                if isinstance(item, dict) and item.get("server") and item.get("agent_role_prompt"):
                    candidate = item
                    break
            # If list contains a single JSON string, try parsing it
            if candidate is None and len(agent_dict) == 1 and isinstance(agent_dict[0], str):
                try:
                    parsed = json.loads(agent_dict[0])
        ...
        if candidate and candidate.get("server") and candidate.get("agent_role_prompt"):
            return candidate["server"], candidate["agent_role_prompt"]
