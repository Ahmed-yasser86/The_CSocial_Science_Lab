...
async def get_search_results(
    ...
    # Run the blocking search in a thread with a per-retriever timeout to avoid hangs
    try:
        retriever_timeout = int(os.environ.get("RETRIEVER_TIMEOUT", "60"))
    except Exception:
        retriever_timeout = 60

    logger.debug("get_search_results: running retriever %s for query '%s' with timeout %s", getattr(retriever, "__name__", str(retriever)), query, retriever_timeout)
    try:
        results = await asyncio.wait_for(
            asyncio.to_thread(search_retriever.search, **search_kwargs), timeout=retriever_timeout
        )
        logger.debug("get_search_results: retriever returned %s results", len(results) if results is not None else 0)
        return results
    except asyncio.TimeoutError:
        logger.error("get_search_results: retriever timeout after %s seconds for query '%s'", retriever_timeout, query)
        raise
    except asyncio.CancelledError:
        logger.warning("get_search_results: task cancelled for query '%s'", query)
        raise
    except Exception as e:
        logger.exception("get_search_results: retriever raised exception: %s", e)
        raise
