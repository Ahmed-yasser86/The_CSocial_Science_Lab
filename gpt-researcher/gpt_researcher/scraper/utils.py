...
def decode_html_bytes(content: bytes, fallback_encoding: str | None = None) -> str:
    """Decode HTML bytes gracefully, even when the page uses a non-standard charset.

    The scraper should never crash because a page contains a few invalid bytes.
    We try the declared/guessed charset first, then a small set of common fallbacks,
    and finally fall back to replacement decoding so the parser can still ingest the page.
    """
    if not content:
        return ""
    ...
    encodings.extend(["utf-8", "utf-8-sig", "cp1252", "latin-1"])
