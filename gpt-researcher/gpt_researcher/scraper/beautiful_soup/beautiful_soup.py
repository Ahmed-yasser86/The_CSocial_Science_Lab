...
class BeautifulSoupScraper:
    ...
    def scrape(self):
        ...
        try:
            ...
            html_text = decode_html_bytes(response.content, declared_encoding)

            # Choose parser intelligently: if the document looks like XML (RSS/Atom or starts with <?xml),
            # use an XML parser to avoid BeautifulSoup's XMLParsedAsHTMLWarning and to parse reliably.
            snippet = (html_text or '').lstrip()[:400].lower()
            looks_like_xml = False
            try:
                if snippet.startswith('<?xml'):
                    looks_like_xml = True
                elif snippet.startswith('<rss') or snippet.startswith('<feed') or snippet.startswith('<rdf'):
                    looks_like_xml = True
                elif 'xmlns="' in snippet and '<html' not in snippet:
                    looks_like_xml = True
                elif re.match(r'^<\w+', snippet) and '<html' not in snippet:
                    # Generic heuristic: starts with a tag and no <html> found in the snippet
                    looks_like_xml = True
            except Exception:
                looks_like_xml = False
            ...
            # If the parser choice still triggers the XMLParsedAsHTMLWarning for some documents,
            # filter that warning locally to avoid noisy logs while preserving other warnings.
