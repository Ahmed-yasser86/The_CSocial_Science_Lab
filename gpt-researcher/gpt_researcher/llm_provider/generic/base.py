...
class GenericLLMProvider:
    ...
    def _sanitize_output(self, text: str) -> str:
        """Strip reasoning/debug tags that some models (deepseek/ollama/qwen) emit
        such as <think>...</think> or other internal reasoning markers so the
        final responses do not contain model-internal thoughts.
        """
        if not isinstance(text, str):
            return text
        # Remove explicit <think>...</think> blocks (DOTALL)
        text = re.sub(r"<think>.*?</think>", "", text, flags=re.DOTALL | re.IGNORECASE)
        # Remove common reasoning tag variants
    ...
    async def get_chat_response(self, messages, stream, websocket=None, **kwargs):
        ...
        # Sanitize model output to remove internal reasoning tags that leak
        res = self._sanitize_output(res)
    ...
    async def stream_response(self, messages, websocket=None, **kwargs):
        ...
        stream_iterator = self.llm.astream(messages, **kwargs)
        try:
            async for chunk in stream_iterator:
                ...
                if not content:
                    continue
                # sanitize streaming chunk to avoid leaking <think> blocks
            ...
            # Ensure async iterator cleanup on task cancellation.
