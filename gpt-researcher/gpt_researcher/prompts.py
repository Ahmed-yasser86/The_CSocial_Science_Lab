...
def extract_smart_query_summary(query: str) -> str:
    """
    Intelligently extract the core intent, task, subject, and objective from large, 
    multi-page research queries/prompts to provide a clean, high-signal query summary for LLM tool selection.
    """
    if not query:
        return ""
    ...
    task_match = re.search(r'(?:TASK|TASK STATEMENT|OBJECTIVE):\s*(.*?)(?=\n\n|\n[A-Z_]+:|\n#|$)', text, re.IGNORECASE | re.DOTALL)
    if task_match:
        extracted_parts.append(f"Task: {task_match.group(1).strip()[:300]}")

    # Extract Subject if present
    subject_match = re.search(r'(?:Subject|Target|Entity):\s*(.*?)(?=\n|$)', text, re.IGNORECASE)
    if subject_match:
        extracted_parts.append(f"Subject: {subject_match.group(1).strip()[:200]}")

    # Extract Main Headings or Core Subject Lines
    heading_matches = re.findall(r'#+\s*(.*?)(?=\n|$)', text)
    if heading_matches:
        clean_headings = [h.strip() for h in heading_matches[:2] if len(h.strip()) > 3]
        if clean_headings:
            extracted_parts.append(f"Key Topics: {'; '.join(clean_headings)}")

    if extracted_parts:
        summary = " | ".join(extracted_parts)
        if len(summary) >= 30:
            return summary

    # Smart fallback: intro + conclusion summary
    first_part = text[:400].strip()
    last_part = text[-300:].strip() if len(text) > 700 else ""
    if last_part and last_part not in first_part:
        return f"{first_part}\n... [Content Brief] ...\n{last_part}"
    
    return first_part


async def async_summarize_query_with_llm(query: str, cfg=None) -> str:
    """
    Uses a fast AI model (like fast_llm) to summarize long multi-page 
    research queries into a thorough, comprehensive research summary.
    Falls back to smart regex extraction if LLM call fails or times out.
    """
    if not query or len(str(query).strip()) <= 500:
    ...
    try:
        ...
        model = getattr(cfg, "fast_llm_model", None) if cfg else None
        provider = getattr(cfg, "fast_llm_provider", None) if cfg else None
        ...
        if not model:
            print(f"⚡ [QuerySummarizer] No fast_llm model configured, using smart regex extraction ({len(text)} chars)")
            return extract_smart_query_summary(query)

        print(f"🧠 [QuerySummarizer] Summarizing research query ({len(text)} chars) using fast model '{model}'...")

        prompt = f"""You are an expert Audience Intelligence & Influence System Query Analyzer.
Your task is to analyze the following detailed research request and extract a high-precision, multi-dimensional research summary specifically structured for audience ecosystem modeling, influence topology, and research tool selection.

STRUCTURE THE SUMMARY AROUND THESE CORE PILLARS:
1. SUBJECT IDENTITY & WORLDVIEW: Subject entity, epistemic posture, core ontology, authority theory, and key societal/institutional stances.
2. AUDIENCE ECOSYSTEM & SEGMENTATION: Targeted audience segments, trust mechanisms, behavioral signatures, and in-group/out-group dynamics.
3. DISSEMINATION TOPOLOGY & INFLUENCE PATHWAYS: Platform channels (YouTube, Telegram, WhatsApp), key network nodes, clip ecosystems, and feedback loops (e.g. suppression/martyrization loops).
4. RHETORICAL & COMMUNICATION SYSTEM: Symbolic vocabulary, framing strategies, register shifts, and authority-construction rituals.
5. PRIMARY RESEARCH OBJECTIVES & GAPS: Specific investigation targets, empirical gaps to resolve, and key analytical deliverables required.

INSTRUCTIONS:
- Retain all domain-specific terminology, subject names, specific analytical targets, and critical investigation scope.
- Exclude administrative instructions, system role prompts, output formatting guidelines, or legal disclaimers.
- Keep the summary highly concise, dense, and high-signal (target under 2,500 characters / max 400 words). Focus sharply on the core intent and specific research goals.

DETAILED RESEARCH REQUEST:
"{text}"

STRUCTURED AUDIENCE INTELLIGENCE SUMMARY:"""

        timeout_val = float(os.environ.get("QUERY_SUMMARIZER_TIMEOUT", "10000.0"))
        print(f"⏱️ [QuerySummarizer] timeout={timeout_val}s")
        summary = await asyncio.wait_for(
            create_chat_completion(
                ...
                llm_kwargs=getattr(cfg, "llm_kwargs", None) if cfg else None,
            ),
            timeout=timeout_val
        )
        if summary and len(summary.strip()) > 10:
            clean_summary = summary.strip()
            preview = clean_summary[:150].replace('\n', ' ')
            print(f"✅ [QuerySummarizer] AI Summarization succeeded! ({len(text)} chars -> {len(clean_summary)} chars)")
            print(f"   Summary Preview: {preview}...")
            return clean_summary
    except Exception as e:
        err_msg = repr(e) if not str(e) else str(e)
        print(f"⚠️ [QuerySummarizer] AI Summarization fallback triggered ({err_msg}). Using regex extraction.")
...
class PromptFamily:
    ...
    @staticmethod
    def generate_mcp_tool_selection_prompt(query: str, tools_info: List[Dict], max_tools: int = 5) -> str:
        """
